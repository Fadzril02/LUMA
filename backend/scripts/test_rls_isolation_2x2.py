...
def ensure_user(email, password, metadata):
    if email in existing_users:
        uid = existing_users[email]
        # update password to ensure we can log in
        admin.auth.admin.update_user_by_id(uid, {"password": password, "user_metadata": metadata})
        return uid
...
s1_uid = ensure_user(s1_email, s1_pass, {"role": "student", "matric_no": s1_matric, "full_name": "Audit Student 1"})
s2_uid = ensure_user(s2_email, s2_pass, {"role": "student", "matric_no": s2_matric, "full_name": "Audit Student 2"})
a1_uid = ensure_user(a1_email, a1_pass, {"role": "advisor", "staff_id": a1_staff, "full_name": "Audit Advisor 1"})
a2_uid = ensure_user(a2_email, a2_pass, {"role": "advisor", "staff_id": a2_staff, "full_name": "Audit Advisor 2"})

print(f"Auth User S1: {s1_email} -> UID {s1_uid}")
print(f"Auth User S2: {s2_email} -> UID {s2_uid}")
print(f"Auth User A1: {a1_email} -> UID {a1_uid}")
print(f"Auth User A2: {a2_email} -> UID {a2_uid}")

# 2. Clean existing records in correct FK sequence
...
print(f"Seed advisor 1 ({a1_staff}) -> Status {r_a1.status_code}")
...
print(f"Seed advisor 2 ({a2_staff}) -> Status {r_a2.status_code}")
...
print(f"Seed student 1 ({s1_matric}) -> Status {r_s1.status_code}")
...
print(f"Seed student 2 ({s2_matric}) -> Status {r_s2.status_code}")
...
print(f"Seed academic record S1 -> Status {r_ar1.status_code}")
...
print(f"Seed academic record S2 -> Status {r_ar2.status_code}")
